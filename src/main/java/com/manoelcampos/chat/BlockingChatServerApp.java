...
public class BlockingChatServerApp {
    ...
    private void clientMessageLoop(final ClientSocket clientSocket){
        try {
            ...
            while((msg = clientSocket.getMessage()) != null){
                ...
                System.out.println("Server socket recebendo mensangem " + msg);
                ...
                if(clientSocket.getLogin() == null){
                    ...
                    //Verificar se a mensagem inicia com @
                    //Filtrar o destinatário da mensagem privada                    
                    String nome = msg.replace("@", "");
                    String[] cliente = nome.split(" ");
                    String nomeCliente = cliente[0];

                    String mensagem = msg.replace("@"+nomeCliente,"");

                    sendMsgPrivate(clientSocket,nomeCliente,mensagem);
    ...
    private void sendMsgPrivate(ClientSocket sender,String nomeCliente, String msg) {
   
        final Iterator<ClientSocket> iterator = clientSocketList.iterator();
        int count = 0;
          while (iterator.hasNext()) {
            final ClientSocket client = iterator.next();
            if(client.getLogin().equals(nomeCliente)){
            
            client.sendMsg("Mensagem privada - " + sender.getLogin() + " diz: " + msg);
